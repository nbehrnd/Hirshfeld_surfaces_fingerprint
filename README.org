...
* Background
  ...
  interactions of a molecule with its surrounding crystal.  These are
  then visualized as Hirshfeld surfaces,[fn:2] and may be projected as
  a 2D fingerprint map.[fn:3]
...
** Computation of the fingerprint maps
   ...
   apexes' $d_i$ to yield the triangle's $d_i$ value.  Similar to this,
   the $d_e$ value representative for the triangle is the average of the
   three apex $d_e$.

   To yield a fingerprint map, the individual triangle areas are
   binned along both d_i and d_e with an increment of 0.01 \AA.  The
   bin-wise sums of individual triangle areas are then normalized
